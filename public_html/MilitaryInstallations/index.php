...
$db = new mysqli('localhost', 'thackbarth', 'og1cee1E', 'thackbarth');
...
unset($Coords);
 ...
 if(isset($_POST['MilitaryBases'])){
          foreach($_POST['MilitaryBases'] as $Mid){
                        $result = $con->query("SELECT asText(SHAPE) as bases
                                                FROM `Militarybases`
                                               WHERE fullname = '{$Mid}'");

                        $Result = $result->fetch_assoc();
						
                        $CoordsArray[] = sql_to_coordinates($Result['bases']);
						print_r($CoordsArray);
               }
             $PrintMap = true;
        }
...
<html>
  <head>
	...
	function initialize() {
	  ...
	  var mapOptions = {
		zoom: 5,
		center: new google.maps.LatLng(33.847090817360645, -98.04199755191803),
